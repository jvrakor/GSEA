# this is a Python GSEA implementation. Imput files are leukemia.txt and pathways.txt. The implementation follows the original article
...
# It is a table containing pathway names, ES(S) values and nominal P values.
...
for i, es in enumerate(ESS):
	Pvalues.append(float(np.where(ESNull[i]==es)[0][0])/N) #Nominal P-values computed from the left portion of the histogram

f=open('gsea.csv', 'w')
f.write('Pathway, ES(S), Nominal P value'+'\n')
for i in np.argsort(Pvalues)[::-1]:
	f.write(str(PathwayName[i])+','+str(ESS[i])+','+str(Pvalues[i])+'\n')
f.close()
